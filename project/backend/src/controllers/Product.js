    ...
    form.parse(req, async (err, fields, files) => {
      if (!err) {
        ...
        if (parsedData.title.trim().length === 0) {
          errors.push({ msg: 'Ít nhất phải có tên sản phẩm!' });
        }
        if (parseInt(parsedData.price) < 1) {
          errors.push({ msg: 'Giá tiền phải lớn hơn 1 VND!' });
        }
        if (parseInt(parsedData.discount) < 0) {
          errors.push({ msg: 'Số phần trăm (%) giảm giá phải là số dương!' });
        }
        if (parseInt(parsedData.stock) < 20) {
          errors.push({ msg: 'Kho hàng phải chứa trên 20!' });
        }
        if (fields.description.trim().length === 0) {
          errors.push({ msg: 'Mô tả về sản phẩm là bắt buộc' });
        }
        if (errors.length === 0) {
          if (!files['image1']) {
            errors.push({ msg: 'Image1 là bắt buộc!' });
          }
          if (!files['image2']) {
            errors.push({ msg: 'Image2 là bắt buộc!' });
          }
          if (!files['image3']) {
            errors.push({ msg: 'Image3 là bắt buộc!' });
          }
          if (errors.length === 0) {
            const images = {};
            ...
            if (errors.length === 0) {
              try {
                ...
                return res
                  .status(201)
                  .json({ msg: 'Tạo sản phẩm thành công!', response });
              } catch (error) {
                console.log(error);
